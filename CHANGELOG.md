...
- Make `secrets.mode=existing` strictly read-only: the chart never creates,
  patches, annotates, or deletes the external database/Redis Secret. The
  bootstrap Job is not rendered in this mode and no Role grants a write verb on
  those Secrets. Publishing `DATABASE_URL` into an external Secret now requires
  the explicit `secrets.database.allowBootstrapPatch=true` opt-in, and
  `databaseProvisioning.enabled=true` with `existing` mode fails validation
  without it.
- Preserve 0.2.0 immutable Deployment selectors. `0.2.1` had added
  ...
  metadata. A static test renders the `affine-0.2.0` tag and diffs the
  selectors.
...
- Bootstrap now patches only `.data.DATABASE_URL` with
  `kubectl patch --type=merge --patch-file`, verifies the result without
  printing it, preserves unrelated keys, and removes any legacy
...
- Add instance labels and instance-scoped selectors to all workloads.
